...
var activeElement;
// This checks for changed data entry controls on the form.  If called as
// part of the OnBeforeUnload event, and if there are changes, it causes the
// browser to prompt the user if they want to leave and lose their changes.
// OnSubmit calls this to update the state of the Dirty flag.  It passes a
// parameter so that it won't attempt to set the prompt.  BP_strFormName
// should be rendered by the page and should contain the form name.
function BP_funCheckForChanges(bInOnSubmit)
{
    ...
    // This prevents a double prompt that can occur when post back
    // is not cancelled for a hyperlink-type control.  Not sure why it
    // happens but this works around it.
...
window.onbeforeunload = function CallCheck(evt)
{
        ...
        if (evt)
        {
                var myReturn = BP_funCheckForChanges();
                if (myReturn != null)
                        evt.returnValue = myReturn;
        }
}
// Firefox Only
